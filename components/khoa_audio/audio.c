...
static const sound_data_t SOUND_MAP[SOUND_MAX] = {
    [SOUND_READY]               = SOUND_DATA(boot),            
    [SOUND_BEEP]                = SOUND_EMPTY,           
    [SOUND_FINGER_DETECTED]     = SOUND_EMPTY, 
    ...
    [SOUND_ENROLL_OK]           = SOUND_DATA(enroll_ok),    
    [SOUND_ENROLL_FAIL]         = SOUND_DATA(match_fail), 
    [SOUND_DELETE_OK]           = SOUND_DATA(delete_ok),
    [SOUND_ERROR]               = SOUND_DATA(match_fail),
};

/* ============================================================================
 * Fallback Tones if MP3 not available
 * ============================================================================ */
typedef struct {
    uint16_t freq;
